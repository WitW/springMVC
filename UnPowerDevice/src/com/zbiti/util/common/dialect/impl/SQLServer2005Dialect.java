package com.zbiti.util.common.dialect.impl;

import com.zbiti.util.common.dialect.Dialect;
...
public class SQLServer2005Dialect implements Dialect {
	...
	return new StringBuffer( sql.length() + 100 ).append( "with tempPagination as(" ).append( sqlRemoveOrderBy )
		.insert( insertPoint + 23, " ROW_NUMBER() OVER(ORDER BY " + sqlOrderBy + ") as RowNumber," ).append(
			") select * from tempPagination where RowNumber between " + begin + " and " + end ).toString();
    }

    public String getLimitString( String sql, int offset, int limit ) {
	setSupportsVariableLimit( offset > 0 );
	if (offset == 0) {
	    return new StringBuffer( sql.length() + 8 ).append( sql ).insert( getSqlAfterSelectInsertPoint( sql ),
		    " top " + limit ).toString();
	}
