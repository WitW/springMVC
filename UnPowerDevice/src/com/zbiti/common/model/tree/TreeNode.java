...
public class TreeNode {
	 ...
	 * @param flag
	 */
	public TreeNode(List<TreeNode> tns,boolean flag) {
		// 将列表转换成以ID为键的Map
		Map<String, TreeNode> tnMaps = new HashMap<String, TreeNode>();
		for (TreeNode n : tns) {
			tnMaps.put(n.getId(), n);
	 ...
	 //对一级和二级排序
		if(flag)
		{
			 if(this.getSonList() != null &&this.getSonList().size() >0)
			 {
			 TreeNode node =  this.getSonList().get(0);
			 List<TreeNode>  nodeList = node.getSonList();
			 ...
			 if(nodeList != null && nodeList.size()>0 )
			 {
				...
				//二级菜单排序
				  for (TreeNode treeNode : nodeList) {
					  ...
					  //一級菜單還有二級菜單
					  if(treeNode.getSonList() != null && treeNode.getSonList().size() >0)
					  {	  
					  Collections.sort(treeNode.getSonList(),new Comparator<TreeNode>() {
